<template>
    ...
    <ul class="nav nav-tabs mt-4 mb-2">
        <li class="nav-item" v-for="c in pod?.spec?.containers?.map(c => c.name)">
            <NuxtLink class="nav-link" aria-current="page" :class="{ active: c === route.query.container }"
                :href="`/pod-logs?name=${route.query.name}&namespace=${route.query.namespace}&container=${c}`">{{
                    c
                }}</NuxtLink>
        </li>
    </ul>
    <div class="form-check form-switch float-end" @click="handlePreviousClick" v-if="hasPrevious(route.query.container)">
        <input class="form-check-input" type="checkbox" role="switch" id="isPrevious"
...
const hasPrevious = (container) => {
    return pod.value?.status.containerStatuses.find(s => s.name === container)?.restartCount > 0
}
